...
def generate_post(hook_strategy, first_line, topic):
    # 「HSPの私」という表現を禁止し、より自然な当事者目線に修正
    prompt = f"""以下の条件でThreadsの投稿文を1つ作成してください。

【あなたの立場：厳守事項】
- あなたはHSPの当事者ですが、文中で「HSPの私は〜」「HSPの私の友人は〜」といった説明臭い自己紹介は一切不要です。
- 「就職支援」「アドバイザー」といったプロ設定は絶対に禁止。嘘は書かないでください。
- あくまで日常の独り言や、同じ悩みを持つ仲間に向けた等身大のメッセージにしてください。
- 支援者目線ではなく、同じ目線の「共感」や「気づき」を大切にしてください。
