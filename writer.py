...
def generate_post(hook_strategy, first_line, topic):
    # 「HSPに寄り添うサポーター」としての立場を明確化
    prompt = f"""以下の条件でThreadsの投稿文を1つ作成してください。

【あなたの立場：厳守事項】
- あなたは「HSPに寄り添うサポーター」です。
- 同じ悩みを持つ人々を支え、ポジティブな気づきを与えるサポーターとしての視点で書いてください。
- ただし、「就職支援のプロ」「アドバイザー」といった嘘の経歴や実績を語ることは絶対に禁止です。
- 文中で「HSPの私は〜」といった説明は不要ですが、サポーターとしての温かさと誠実さを込めてください。
